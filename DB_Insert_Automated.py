...
for each in dailyList:
    ...
    if each not in todayDoneList:
        ...
        newObj = NewReconUpdate(ReconUser=mem.ReconUser,ReconTitle=mem.ReconTitle,ReconTotalCount=0,ReconExecutedCount=0,ReconType="Daily",ReconDateTime=(datetime.datetime.now()+ datetime.timedelta(hours=4,minutes=30)),ReconComments=" ")
        newObj.save()
